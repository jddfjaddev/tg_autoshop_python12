...
def start_bot():
    ...
    def handler_call(call):
        ...
        if call.data == 'ref_system':
            ...
            try:
                msg = f"""Реф система:
                        \n<iЕсли человек, присоединившийся к боту по вашей ссылке , пополнит баланс, то вы получите:</i><b> {db.get_value('referral_percent')}%</b>
        ...
        if call.data in func.product_list():
            ...
            if count > 0:
                text = f'➡Название: {product[2]}\n{product[3]}\nСтоимость: {product[4]} {db.get_value("money_value", base="config")}'
            else:
        ...
        if call.data == 'exit_to_adm_menu':
            if func.is_adm(call.message.chat.id):
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                          text='<b> Вы вернулись в админ меню!</b>', reply_markup=menu.adm_menu,
                                          parse_mode='HTML')
                except Exception as e:
                    print(e)
        ...
        if call.data == 'exit_to_kur_menu':
            if func.is_kur(call.message.chat.id):
                try:
                    bot.edit_message_text(chat_id=chat_id, message_id=message_id,
                                          text='<b>♻ Вы вернулись в меню Курьера!</b>', reply_markup=menu.kur_menu,
                                          parse_mode='HTML')
                except Exception as e:
                    print(e)
