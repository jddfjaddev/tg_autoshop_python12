...
def kuna_code(user_id):
    ...
    * UAH \n
    ➖➖➖➖➖➖➖➖➖➖\n
    1️⃣ Через любой терминал EasyPay - выберите меню>> .... 
    >> выбираем Kuno Code >> 
    ...
    2️⃣ Через сайт Kuna.io (Быстрая регистрация через почту)  >> 
...
def add_adm(message):
    ...
    if db.get_value('*', 'user_id', id, 'users') is not None:
        db.add_adm(id)
        bot.send_message(chat_id=message.chat.id, text=f'Выдал admin пользователю {id}', reply_markup=menu.adm_menu)
    else:
...
def remove_adm(message):
    ...
    if db.get_value('*', 'user_id', id, 'users') is not None:
        db.remove_adm(id)
        bot.send_message(chat_id=message.chat.id, text=f'Удалил admin {id}', reply_markup=menu.adm_menu)
    else:
        bot.send_message(chat_id=message.chat.id, text=f'Нет такого пользователя в базе', reply_markup=menu.adm_menu)
...
def add_product(message, id):
    ...
    bot.send_message(chat_id=config.CHAT_GROUP_ID, text='✅ Курьер успешно добавил содержимое к товару')
